const URL_SHEET = "https://script.google.com/macros/s/AKfycbzdXO7rJe848X5WqcZ7O36gP0zgXIc9YkzOgh0R6hj528fzuZB_hp6FNITyv-9TwJOB3A/exec";
const PRECIO = 20;
let picks = Array(9).fill("");
let quinielas = JSON.parse(localStorage.getItem("quinielas_golazo")||"[]");

window.selection = function(el){
  let id=el.id; let idx=parseInt(id.substring(1))-1;
...
window.random = function(){
  clean(); for(let i=0;i<9;i++){ let ops=["L","E","V"]; let r=ops[Math.floor(Math.random()*3)]; let el=document.getElementById(r+(i+1)); if(el) selection(el); }
}
...
window.send = function(){
  ...
  // 2. WHATSAPP
  let txt=`*QUINIELA GOLAZO*%0A`; quinielas.forEach((it,i)=>{ txt+=`${i+1}. ${it.q.join(" ")} - ${it.nombre}%0A`; });
  txt+=`%0ACOSTO: $${costo}%0A`;
  let wa="https://wa.me/524776482132?text="+txt; window.open(wa,"_blank");
